...
export function SocialsBag({ position, rotation, delay }: SocialsBagProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Social icons that peek out from the top of the bag */}
            ...
            {isHovered &&
              socials.map((social, index) => (
                <motion.a
                  ...
                  className="w-7 h-7 rounded-full flex items-center justify-center shadow-lg cursor-pointer flex-shrink-0"
                  style={{ background: social.bg }}
                  initial={{ y: 24, opacity: 0, scale: 0.5 }}
                  animate={{ y: 0, opacity: 1, scale: 1 }}
                  exit={{ y: 24, opacity: 0, scale: 0.5 }}
                  transition={{
                    type: "spring",
                    stiffness: 400,
                    damping: 15,
